...
export function handleChatMessages(raw: RawData) {
	...
	try {
		void addMessageDB(msg).catch((error: any) => {
			console.log("[WS] ", error.message);
		});
		if (msg.type === "block" && msg.receiver) {
			void blockUserDB(msg.sender, msg.receiver).catch((error: any) => {
				console.log("[WS] ", error.message);
			});
		} else if (msg.type === "unblock" && msg.receiver) {
			void unblockUserDB(msg.sender, msg.receiver).catch((error: any) => {
				console.log("[WS] ", error.message);
			});
		}
