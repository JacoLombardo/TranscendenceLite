...
export default async function userRoutes(fastify: FastifyInstance) {
	...
	fastify.get("/api/user/data", async (request: FastifyRequest, reply: FastifyReply) => {
		...
		try {
			const userPayload: userData = {
				user: username,
				chatHistory: await buildChatHistory(username),
				friends: await getUserFriendsDB(username),
				blockedUsers: await getBlockedUsersDB(username),
			};
	...
	fastify.get("/api/user/me", async (request: FastifyRequest, reply: FastifyReply) => {
		...
		// Look up the user by username in the database
		try {
			const user = await getUserByUsernameDB(payload.username);
