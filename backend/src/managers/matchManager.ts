...
export function createMatch({
	...
	try {
		void createMatchDB(match).catch((error: any) => {
...
export function startMatch(match: Match) {
	try {
		if (match.singleGameId && match.mode === "local") {
			void startMatchDB(match.id).catch((error: any) => {
				console.error("[MM]", error.message);
			});
			...
			void startMatchDB(match.id, match.tournament.id).catch((error: any) => {
				console.error("[MM]", error.message);
			});
...
export function endMatch(match: Match) {
	...
	void endMatchDB(match).catch((error: any) => {
		console.error("[MM]", error.message);
	}); // Update database
...
export function addPlayerToMatch(match: Match, playerId: string, socket: any, displayName?: string) {
	try {
		...
		if (!match.players.left) {
			void addPlayerMatchDB(match.id, playerId, "left").catch((error: any) => {
				console.error("[MM]", error.message);
			});
			match.players.left = {
				username: playerId,
			...
			void addPlayerMatchDB(match.id, playerId, "right").catch((error: any) => {
				console.error("[MM]", error.message);
			});
			match.players.right = {
				username: playerId,
	...
	if (match.singleGameId) {
		...
		if (!isMatchFull && !isMatchStarted) {
			...
			if (match.players.left?.username === playerId) {
				...
				void removePlayerFromMatchDB(match.id, playerId, "left").catch((error: any) => {
					console.error("[MM]", error.message);
				});
			} else if (match.players.right?.username === playerId) {
				match.players.right = undefined;
				void removePlayerFromMatchDB(match.id, playerId, "right").catch((error: any) => {
					console.error("[MM]", error.message);
				});
			}
			return;
