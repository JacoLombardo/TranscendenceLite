...
const remoteGameCounters = new Map<string, number>();
...
export function getOrCreateSingleGame(id: string, mode: string, creator?: string): SingleGame {
	...
	if (!singleGame) {
		...
		if (creator && mode === "remote") {
			const current = remoteGameCounters.get(creator) ?? 0;
			const counter = current + 1;
			remoteGameCounters.set(creator, counter);
				...
				singleGame.expirationTimer = setTimeout(
					(singleGame: SingleGame) => {
						if (!checkMatchFull(singleGame.match)) {
							...
							void removeMatchDB(singleGame.match.id).catch((error: any) => {
