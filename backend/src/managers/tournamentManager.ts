...
export async function getOrCreateTournament({
	...
	if (!tournament) {
		...
		if (creator && name) {
			...
			const counter = (await getTournamentCountByCreator(creator)) + 1;
		...
		void createTournamentDB(newTournament.id, newTournament.name, newTournament.state.size, creator).catch(
			(error: any) => {
				console.log(
					`[TM] Tournament ${newTournament.id} already exists in database or save failed:`,
					error.message
				);
			}
		);
		...
		const matches = initTournamentMatches(newTournament, newTournament.state.size);
		newTournament.matches.set(1, matches);

		// Set timer to wait for players
		newTournament.expirationTimer = setTimeout(
			(tournament: Tournament) => {
				if (!checkTournamentFull(tournament)) {
					...
					void removeTournamentDB(tournament.id).catch((error: any) => {
						console.error("[TM]", error.message);
					}); // Remove tournament and its matches from database
		...
		console.log(
			`[TM] NEW OPEN TOURNAMENT: ${newTournament.id} (${newTournament.name}) - ${newTournament.state.size} players`
		);
	}
	return tournament;
}
...
export function startTournament(tournament: Tournament) {
	try {
		...
		if (matches) {
			...
			void startTournamentDB(tournament.id).catch((error: any) => {
				console.error("[TM]", error.message);
			});
				...
				if (!checkMatchFull(match)) {
					if (tournament.state.round === 1 && playerDisplayName && socket) {
						...
						if (!alreadyInTournament) {
							...
							void createTournamentPlayerDB(tournament.id, playerId, playerDisplayName).catch(
								(error: any) => {
									console.error("[TM]", error.message);
								}
							);
...
export function endTournament(tournament: Tournament) {
	...
	void endTournamentDB(tournament.id, tournamentWinner).catch((error: any) => {
		console.error("[TM]", error.message);
	});
...
export function forfeitTournament(tournamentId: string, playerId: string) {
	...
	if (tournament && !tournament.state.isOver) {
		...
		try {
			void removeTournamentPlayerDB(tournament.id, playerId).catch((error: any) => {
				console.error("[TM]", error.message);
			});
		} catch (error: any) {
		...
		if (tournament.state.isRunning) {
			...
			if (roundMatches) {
				for (const match of roundMatches) {
					...
					void forfeitMatchDB(match.id, playerId).catch((error: any) => {
						console.error("[TM]", error.message);
					});
				}
			}
			void forfeitTournamentDB(tournament.id, playerId).catch((error: any) => {
				console.error("[TM]", error.message);
			});
			tournaments.delete(tournament.id);
		}
