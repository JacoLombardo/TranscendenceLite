...
export function isGameOver(match: Match): void {
	...
	if (state.isOver) {
		//  added this to track when match ended for delayed reset handling (so final score is visible)
...
export function stepMatch(match: Match, dt: number): void {
	...
	if (ball.x < -GAME_CONSTANTS.SCORE_OUT_MARGIN) {
		state.score.right += 1;
		void updateMatchDB(match.id, state.score.left, state.score.right).catch((error: any) => {
			console.error("[DB] Failed to update match score:", error.message);
		});
