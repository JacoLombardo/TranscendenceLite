...
A full-stack multiplayer Pong web application built as part of the 42 curriculum. Players can compete in real-time matches or tournaments, chat with other users, and use GitHub OAuth authentication with profiles, friends/blocked lists, and three languages.
...
| **Database** | PostgreSQL (Neon) | Managed Postgres with persistent storage and zero-ops setup for demo deployments |
...
│ score_left │ score_right │ winner │ started_at │ ended_at │
└─────────────────────────────────────────────────────────────────────┘
...
- Open the frontend URL and wait for the lobby to load.
- Create two users (or log in with two browsers/incognito).
- Start a remote game and verify real-time paddle movement.
- Create a tournament and verify bracket updates across both clients.
- Send a direct message and confirm it appears for the recipient.
- Refresh the page and verify session persists (cookie-based).
...
| `PGSSLMODE`       | `require`               | SSL mode for hosted Postgres |
